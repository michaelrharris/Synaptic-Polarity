...
for qq in range(10):  
    ...
    for x in range(len(tenfold[qq])):
        ...
        c[tenfold[qq][x][0]][tenfold[qq][x][1]] = 0
    ...
    for x in range(len(A)):
        for y in range(len(A)):
            if (x,y) in links:
                if A[x][y] < 0:
                    dfupdate1 = pd.DataFrame([[abs(A[x][y]),x,y,"neg"]],columns=['value','pos1','pos2','sign'])
                    presults = presults.append(dfupdate1, ignore_index = True)
                elif A[x][y] > 0:
                    dfupdate1 = pd.DataFrame([[abs(A[x][y]),x,y,"pos"]],columns=['value','pos1','pos2','sign'])
                    presults = presults.append(dfupdate1, ignore_index = True)
                elif A[x][y] == 0:
                    dfupdate1 = pd.DataFrame([[abs(A[x][y]),x,y,"zero"]],columns=['value','pos1','pos2','sign'])
                    presults = presults.append(dfupdate1, ignore_index = True)
            else:
                continue
